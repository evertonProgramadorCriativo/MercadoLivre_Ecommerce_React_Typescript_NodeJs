...
type ProductInterface = { name: string; price: number; image: string };

type ThemeContext = {
  activatingSmallDropDown: Theme;
  SmallDropDownManagerButton: () => void;
  smallExitButton: () => void;
  activatingLargeDropDown: Theme;
  LargeDropDownManagerButton: () => void;
  LargeExitButton: () => void;
  SuspendenExit: () => void;
...
export const ThemeContext = React.createContext<ThemeContext>(
  {} as ThemeContext || null
);

export const ThemeReactProvider: React.FC<Props> = ({ children }) => {
  const [activatingSmallDropDown, setActivatingSmallDropDown] =
    useState<Theme>(false);

  const SmallDropDownManagerButton = () => {
    setActivatingSmallDropDown((current) => !current);
    setActivatingSmallDropDown(
      activatingSmallDropDown === false ? true : false
    );
  };
  ...
  return (
    <ThemeContext.Provider
      value={{
        ...
        smallExitButton,
        SmallDropDownManagerButton,
        activatingLargeDropDown,
        LargeExitButton,
        LargeDropDownManagerButton,
        SuspendenExit,
